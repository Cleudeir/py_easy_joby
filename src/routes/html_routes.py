...
def agent_summary_reconstruction_code():
    ...
    if request.method == 'POST':
        ...
        user_prompt_file = request.files['file']
        
        try:
            user_prompt = user_prompt_file.read().decode('utf-8')
            print("".format(user_prompt))
            Response(user_prompt, mimetype='text/html')
        except Exception as e:  # You can catch specific exceptions if needed, like FileNotFoundError
